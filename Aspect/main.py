...
# max_preview = 14
# max_cols = 5
# for i in range(max_preview // max_cols):
#     rows = [row for row in range(max_cols)]
#     print(rows)
# if max_preview % max_cols != 0:
#         print([row for row in range(max_preview % max_cols)])
...
#         first_row[1].container().write('Billing Plan\n\n > **PREMIUM**')
...
#     uploaded_images = st.file_uploader(label='Upload Images Here', accept_multiple_files=True, type=['jpg', 'png', 'jpeg'])

#     st.subheader('Image Preview (First 5 Images)')

#     if uploaded_images:
#         upload_btn = st.button('Upload to Local Directory')
#         if upload_btn:
#             image_container = st.container()
#             with image_container:
#                 img_slots = st.columns(len(uploaded_images[:5]))
#                 for idx, image in enumerate(uploaded_images[:5]):
#                     img_slots[idx].image(image)

#             for image in uploaded_images:
#                 save_uploaded_file(image, st.session_state['project_path'])
...
#     feat_matchers = st.multiselect(
#         'Select Feature Matchers',
#         list(feat_map.keys()),
#         ['LightGlue+Aliked']
#     )

    # st.write(feat_matchers)

    # if st.button('Start Reconstruction', use_container_width=True):
    #     manager = PipelineManager(img_dir = st.session_state['project_path'], 
    #                               device = torch.device('cuda'))
    #     for idx, fm in enumerate(feat_matchers):
    #         manager.create_instance(instance_id=idx, 
    #                                 model_name=feat_map[fm])
    #     reconstruction = manager.run_reconstruction_for_all()
    #     fig = viz_3d.init_figure()
    #     viz_3d.plot_reconstruction(fig, reconstruction, cameras = False, 
    #                                         color = 'rgba(227,168,30,0.5)', cs = 5)
    #     st.plotly_chart(fig)
